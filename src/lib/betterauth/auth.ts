...
export const auth = (platform: Platform) => {
  ...
  let betterAuthOrigin = platform?.env?.BETTER_AUTH_TRUSTETD_ORIGIN || process.env.BETTER_AUTH_TRUSTETD_ORIGIN || '';
  ...
  trustedOrigins.add('https://localhost:8787'); // Cloudflare Workers default dev origin
  // trustedOrigins.add('https://sveltekit_template.lauhard.workers.dev'); // Cloudflare Workers default dev origin
